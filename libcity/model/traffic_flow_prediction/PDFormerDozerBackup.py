...
class STSelfAttention(nn.Module):
    ...
    def forward(self, x, x_patterns, pattern_keys, geo_mask=None, sem_mask=None):
        ...
        #Dozer semantic spatial attention goes here
        '''
        queries = sem_q
        keys = sem_k
        L_Q = T
        L_K = T
        scores = torch.einsum("bthnd,bthmd->bthnm", queries, keys)
        sparse_mask = torch.zeros(N, N, device=scores.device)
        for w_idx in range(self.local_window//2+1):
            sparse_mask = torch.diagonal_scatter(sparse_mask, torch.ones(L_Q - w_idx), w_idx)
            sparse_mask = torch.diagonal_scatter(sparse_mask, torch.ones(L_Q - w_idx), -w_idx)
        stride = self.stride + 1
        for w_idx in range(0, L_Q, stride):
            ...
            sparse_mask = torch.diagonal_scatter(sparse_mask, torch.ones(L_Q - w_idx), -w_idx)
        scores = scores * sparse_mask
        A = self.t_attn_drop(torch.softmax(scale * scores, dim=-1))
        V = torch.einsum("bthnm,bthdn->bnthd", A, sem_v)
        sem_x = V.reshape(B, T, N, int(D * self.sem_ratio))    
        '''
...
class PDFormerDozer(AbstractTrafficStateModel):
    ...
    def forward(self, batchx, batchy, lap_mx=None):
        x = batchx
        T =  x.shape[1]
        x_pattern_list = []
    ...
    def calculate_loss(self, batchx, batchy, lap_mx=None, batches_seen=None):
        y_true = batchy
        y_predicted = self.predict(batchx,batchy,lap_mx)
        return self.calculate_loss_without_predict(y_true, y_predicted, batches_seen)

    def predict(self, batchx, batchy, lap_mx=None):
        return self.forward(batchx, batchy, lap_mx)
