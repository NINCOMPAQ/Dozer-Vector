...
class VFPE(nn.Module):
    def __init__(self, num_nodes, embed_dim):
        super().__init__()
        #self.feature_enc = nn.Linear(2, embed_dim)
    ...
    def forward(self, VF):
        #print("VF shape is: ", VF.shape)
        inter = VF.permute(0, 2,1).float()
        inter = inter.cuda()
        #print('VF shape after permute is: ', inter.shape)
        #print('inter dtype is: ', inter.dtype)
        inter = self.feature_enc(inter)
        #print('VF shape after first encode is: ', inter.shape)
        inter = inter.permute(0, -1,-2)
        inter = self.node_enc(inter)
        inter = inter.permute(0, -1,-2)
        inter = inter.unsqueeze(0)
        inter = inter.permute(0, 3, 1,2)
        #print("VFPE shape is: ", inter.shape)
        return inter


class DataEmbedding(nn.Module):
    def __init__(
        self, feature_dim, embed_dim, lape_dim, adj_mx, num_nodes = 1024, drop=0.,
        ...
        self.spatial_embedding = VFPE(num_nodes, embed_dim)
        self.dropout = nn.Dropout(drop)

    def forward(self, x, lap_mx, VF):
        origin_x = x
...
class STSelfAttention(nn.Module):
    def __init__(
        ...
        attn_drop=0., proj_drop=0., device=torch.device('cpu'), output_dim=1, local_window = 1,stride = 7,
    ):
        super().__init__()
        assert dim % (geo_num_heads + sem_num_heads + t_num_heads) == 0
        self.local_window = local_window #this is new
        self.stride = stride #this is new too
    ...
    def forward(self, x, x_patterns, pattern_keys, geo_mask=None, sem_mask=None):
        ...
        V = torch.einsum("bhntl,bnhtd->bnthd", A, t_v)
        t_x = V.reshape(B, N, T, int(D * self.t_ratio)).transpose(1, 2)
        
        #Dozerattention complete
...
class STEncoderBlock(nn.Module):
    ...
    def forward(self, x, x_patterns, pattern_keys, geo_mask=None, sem_mask=None):
        #Cal_FLOPs(model = self.st_attn, inputs = (self.norm1(x), x_patterns, pattern_keys, geo_mask, sem_mask))
        if self.type_ln == 'pre':
...
class PDFormerDozer(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.VF  = torch.from_numpy(data_feature['VF']).float().to(self.device)

        self.enc_embed_layer = DataEmbedding(
            self.feature_dim - self.ext_dim, self.embed_dim, lape_dim, self.adj_mx, num_nodes = self.num_nodes, drop=drop,
    ...
    def forward(self, batchx, batchy, lap_mx=None, VF=None):
        x = batchx
        T =  x.shape[1]
        x_pattern_list = []
        ...
        enc = self.enc_embed_layer(x, lap_mx, VF)
        skip = 0
    ...
    def calculate_loss(self, batchx, batchy, lap_mx=None, VF=None, batches_seen=None):
        y_true = batchy
        y_predicted = self.predict(batchx,batchy,lap_mx, VF)
        return self.calculate_loss_without_predict(y_true, y_predicted, batches_seen)

    def predict(self, batchx, batchy, lap_mx=None, VF = None):
        return self.forward(batchx, batchy, lap_mx, VF)
