...
class LaplacianPE(nn.Module):
    ...
    def forward(self, lap_mx):
        #print('laplacian shape is: ', lap_mx.shape)
        lap_pos_enc = self.embedding_lap_pos_enc(lap_mx).unsqueeze(0).unsqueeze(0)
        #print('lap_pos_enc shape is : ', lap_pos_enc.shape)
        return lap_pos_enc
...
class DataEmbedding(nn.Module):
    def __init__(
        ...
        self.spatial_embedding2 = LaplacianPE(lape_dim, embed_dim)
        self.spatial_embedding1 = VFPE(num_nodes, embed_dim)
...
class STSelfAttention(nn.Module):
    ...
    def forward(self, x, x_patterns, pattern_keys, geo_mask=None, sem_mask=None):
        ...
        geo_q = geo_q.reshape(B, T, N, self.geo_num_heads, self.head_dim).permute(0, 1, 3, 2, 4) #B, T, Num_Heads, N, D
