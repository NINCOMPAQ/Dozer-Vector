...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x, lap_mx):
        ...
        xs = [x, x]
        #xs[0] is spatial, xs[1] is temporal
        xs[1] += self.position_encoding(x)
        if self.add_time_in_day:
            xs[0] += self.daytime_embedding((origin_x[:, :, :, self.feature_dim] * self.minute_size).round().long())
            xs[1] += self.daytime_embedding((origin_x[:, :, :, self.feature_dim] * self.minute_size).round().long())
        if self.add_day_in_week:
            xs[0] += self.weekday_embedding(origin_x[:, :, :, self.feature_dim + 1: self.feature_dim + 8].argmax(dim=3))
            xs[1] += self.weekday_embedding(origin_x[:, :, :, self.feature_dim + 1: self.feature_dim + 8].argmax(dim=3))
        xs[0] += self.spatial_embedding(lap_mx)
        xs[0] = self.dropout(xs[0])
...
class STSelfAttention(nn.Module):
    ...
    def forward(self, xs, x_patterns, pattern_keys, geo_mask=None, sem_mask=None):
        B, T, N, D = xs[0].shape
        x = xs[1]
...
class STEncoderBlock(nn.Module):
    ...
    def forward(self, xs, x_patterns, pattern_keys, geo_mask=None, sem_mask=None):
        if self.type_ln == 'pre':
            temp_attn = self.drop_path(self.st_attn([self.norm1(xs[0]), self.norm1(xs[1])], x_patterns, pattern_keys, geo_mask=geo_mask, sem_mask=sem_mask))
            xs[0] = xs[0] + temp_attn
            xs[1] = xs[1] + temp_attn
            xs = xs + [self.drop_path(self.mlp(self.norm2(xs[0]))), self.drop_path(self.mlp(self.norm2(xs[1])))]
        elif self.type_ln == 'post':
            temp_attn = self.drop_path(self.st_attn(xs, x_patterns, pattern_keys, geo_mask=geo_mask, sem_mask=sem_mask))
            xs = [self.norm1(xs[0] + temp_attn), self.norm1(xs[1] + temp_attn)]
            xs = [self.norm2(xs[0] + self.drop_path(self.mlp(xs[0]))),self.norm2(xs[1] + self.drop_path(self.mlp(xs[1])))]
        return xs


class PDFormerSpatialPartition(AbstractTrafficStateModel):
    ...
    def forward(self, batch, lap_mx=None):
        ...
        encs = self.enc_embed_layer(x, lap_mx)
        skip = 0
        for i, encoder_block in enumerate(self.encoder_blocks):
            encs = encoder_block(encs, x_patterns, pattern_keys, self.geo_mask, self.sem_mask)
            skip += self.skip_convs[i](encs[0].permute(0, 3, 2, 1))
            skip += self.skip_convs[i](encs[1].permute(0, 3, 2, 1))
